...
export default class PersonalAddressRequest extends BaseClass {
  ...
  saveUser(req: TMerchantUserEntity): ApolloAPIReturnType {
  ...
  getAddress(req: TUserAddressRequest): ApolloAPIReturnType {
    return this.exceptionWrapper(async () => {
      const data: DataBuilder = new DataBuilder(req);
      this.headerBuilder.setData(data);
  ...
  getListOfAddresses(req: TPersonalAddressFilter): ApolloAPIReturnType {
    return this.exceptionWrapper(async () => {
      const data: DataBuilder = new DataBuilder(req);
      this.headerBuilder.setData(data);
  ...
  getUser(id: TNullableString = null, clientId: TNullableString = null): ApolloAPIReturnType {
    return this.exceptionWrapper(async () => {
      if (!clientId && !id) {
